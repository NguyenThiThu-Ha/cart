...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		# 	'slug': category_description['categories_name'],
		# 	'term_group': 0,
		# }
		# categories_terms_query = self.create_insert_query_connector('terms', categories_terms)  # insert into table
		# cate_id = self.import_category_data_connector(categories_terms_query, True, convert['id'])
		# cate_termmeta_value = {
		# 	'order': 0,
		# 	'display_type': 'products',
		# 	'thumbnail_id': cate_id,
		...
		# 		self.import_category_data_connector(self.create_insert_query_connector('termmeta', cate_termmeta), True, convert['id'])
		# if not cate_id:
		# 	return response_warning('term ' + to_str(convert['id']) + ' import false.')
		# self.insert_map(self.TYPE_CATEGORY, convert['id'], cate_id, convert['name'], convert['name'], None, self._notice['src']['language_default'])
		# return response_success(cate_id)
		...
		category_term_import = self.import_category_data_connector(self.create_insert_query_connector('terms', category_data), True, convert['id'])
		category_taxonomy_data = {
			'term_id': category_term_import,
			'taxonomy': 'product_cat',
			'description': convert['description'] if convert['description'] else '',
			'parent': id_parent,
		}

		category_tax_import = self.import_category_data_connector(self.create_insert_query_connector('term_taxonomy', category_taxonomy_data), True, convert['id'])
		if category_term_import:
	...
	def convert_product_export(self, product, products_ext):
		...
		product_price = ''
		if to_decimal(self.get_value_metadata(product_meta, '_msrp', 0.0000)) > 0:
			product_price = self.get_value_metadata(product_meta, '_msrp', 0.0000)
		else:
			product_price = self.get_value_metadata(product_meta, '_msrp', 0.0000)
		...
		sale_price = self.get_value_metadata(product_meta, '_wholesale_price', '')
		if sale_price != '':
		...
		if product_child_src:
			...
			for product_child in product_child_src:
				...
				for attribute in attr_child:
					...
					attribute_child_data['option_value_description'] = get_row_value_from_list_by_field(products_ext['data']['term_relationship'], 'slug', attribute['meta_value'], 'description') if get_row_value_from_list_by_field(products_ext['data']['catelationship'], 'slug', attribute['meta_value'], 'description') else ''
					language_id = self._notice['src']['language_default']
	...
	def check_product_import(self, convert, product, products_ext):
		...
		for attr in convert['attributes']:
			meta_insert = {attr['attribute_code']: {
			...
			all_attr.update(meta_insert)
			position += 1
		data_update = {
				'post_id': product_import,
				'meta_key': '_product_attributes',
				'meta_value': php_serialize(meta_insert)
			}
		where_meta = {
			'post_id': product_import,
			'meta_key': '_product_attributes'
		}
		self.import_product_data_connector(self.create_update_query_connector('postmeta', data_update, where_meta),True, convert['id'])
		return True
	...
	def product_import(self, convert, product, products_ext):
		...
		# bld
		global product_meta_data, variation_post_id_product, variation_meta_data, variation_product_data
		self.log(convert, "convert_product_log")
		all_query = list()

		post_name = self.sanitize_title(convert['name'])
		product_data = {
			'post_date': convert['created_at'] if convert['created_at'] else '',
			'post_date_gmt': convert['created_at'] if convert['created_at'] else '',
			'post_content': convert['description'] if convert['description'] else '',
			'post_title': convert['name'] if convert['name'] else '',
			'post_status': 'publish',
			'post_name': post_name,
			'post_modified': convert['updated_at'] if convert['updated_at'] else '',
			'post_type': 'product',
		}

		if self._notice['config']['pre_prd']:
			product_data['ID'] = convert['id']
		id_product = self.import_product_data_connector(self.create_insert_query_connector('posts', product_data), True, convert['id'])
		self.log(id_product, "id_product_afterinsert")
		if id_product:
		...
		# id_variation_list = []
		#
		# for i, j in variation_posttitle.items():
		...
		# 	}
		# 	variation_post_id_product = self.import_customer_data_connector(self.create_insert_query_connector('posts', variation_product_data), True, convert['id'])
		# 	id_variation_list.append(variation_post_id_product)
		#
		# # self.log(convert['options'][0]['values'][0]['option_value_qty'], "checkvari_qty")
		# variation_qty = {}
		# for count, id in enumerate(id_variation_list):
		# 	variation_qty[id] = convert['options'][0]['values'][count]['option_value_qty']
		#
		# for id, qty in variation_qty.items():
		# 	variation_meta_data = {
		# 		'_tax_status': 'taxable',
		# 		'_tax_class': 'parent',
		# 		'_regular_price': convert['price'],
		# 		'_sale_price': convert['special_price']['price'],
		# 		'_manage_stock': 'yes' if convert['manage_stock'] == True else 'no',
		# 		'_stock': qty,
		...
		# 	}
		# 	for option, value in variation_meta_data.items():
		# 		variation_product_meta_data = {
		# 			'post_id': id,
		# 			'meta_key': option,
		# 			'meta_value': value,
		# 		}
		# 		all_query.append(self.import_customer_data_connector(self.create_insert_query_connector('postmeta', variation_product_meta_data)))

		# id_variation = self.import_customer_data_connector(self.create_insert_query_connector('posts', variation_product_data), True, convert['id'])
		# variation_qty = get_row_from_list_by_field(convert['options'][0]['values'], 'option_value_name', variation_posttitle)
		# variation_meta_data = {
		# 	'_tax_status': 'taxable',
		# 	'_tax_class': 'parent',
		# 	'_regular_price': convert['price'],
		...
		# 	'_manage_stock': 'yes' if convert['manage_stock'] == True else 'no',
		# 	'_stock': variation_qty['option_value_qty'],
		# 	'_stock_status': 'instock' if int(variation_qty['option_value_qty']) > 0 else 'outofstock',
		# 	'_price': convert['price'],
		# 	'_sku': convert['sku'],
		# 	'_weight': convert['weight'],
		# 	'_length': convert['length'],
		...
		# for option, value in variation_meta_data.items():
		# 	variation_product_meta_data = {
		# 		'post_id': id_variation,
		# 		'meta_key': option,
		# 		'meta_value': value,
		# 	}
		# 	all_query.append(self.create_insert_query_connector('postmeta', variation_product_meta_data))


	def after_product_import(self, product_id, convert, product, products_ext):
		products_posts_image = {  # all entity to insert
		...
		products_term_query_image = self.create_insert_query_connector('posts', products_posts_image)  # insert into table
		...
		if thumb[0:4] != 'http':
			products_posts_image_expand['guid'] = self._notice['target']['cart_url'] + '/wp-content/uploads/2007/05/' + product['products_image']
		else:
		...
		data_meta_post_img = {
			'_wp_attached_file': '2007/05/' + product['products_image'],
		}
		data_meta = {
			'post_id': '',
			'meta_key': '',
		...
		if prd_id_image:
			for key, value in data_meta_post.items():
				...
				self.import_product_data_connector(self.create_insert_query_connector('postmeta', data_meta), True, convert['id'])
		...
		# all_queries.append(self.get_query_img_wpml(thumbnail_id, language_code))all_queries.append(self.get_query_img_wpml(thumbnail_id, language_code))
		...
		# self.log(product_id, "after_import_productid")
		# all_queries = list()
		# all_categories = list()
		# if convert['categories']:
		# 	for category in convert['categories']:
		...
		# 		if not category_id:
		# 			category_id = self.get_map_field_by_src(self.TYPE_CATEGORY, None, category['code'])
		# 		if not category_id:
		# 			category_id = self.get_map_field_by_src(self.TYPE_CATEGORY, category['id'], None)
		# 		if id_category :
		# 			all_categories.append(id_category)
		# 	all_categories = list(set(all_categories))
		# 	for cate_id in all_categories:
		# 		category_data = {
		# 			'object_id': product_id,
	...
	def customer_import(self, convert, customer, customers_ext):
		global user_meta_id, user_meta_data
		self.log(convert, "convert_customer_log")
		customer_data = {
			'user_login': convert['first_name'] if convert['first_name'] else '',
			'user_nicename': convert['first_name'] if convert['first_name'] else '',
			...
			'user_status': 1 if convert['active'] else 0,
			'display_name': convert['first_name'] if convert['first_name'] else '',
		}

		customer_id = self.import_customer_data_connector(self.create_insert_query_connector('users', customer_data),
		                                                  True, convert['id'])
		...
		for option, value in metakey_values.items():
			user_meta_data = {
				'user_id': customer_id,
				'meta_key': option,
				'meta_value': value,
			}
			user_meta_id = self.import_customer_data_connector(self.create_insert_query_connector('usermeta', user_meta_data))

		if customer_id and user_meta_id:
	...
	def order_import(self, convert, order, orders_ext):
		...
		order_data = {
		    ...
		    'post_date_gmt' : convert['created_at'] if convert['created_at'] is not None else get_current_time(),
		    'post_content' : '',
			'post_status': 'wc-pending' if convert['status'] == '1' else '',
			'post_modified': convert.get('created_at') if convert.get('created_at') else get_current_time(),
			'post_modified_gmt': convert['created_at'] if convert['created_at'] is not None else get_current_time(),
			'post_type': 'shop_order',
		}

		order_id = self.import_order_data_connector(self.create_insert_query_connector('posts', order_data), True, convert['id'])
		...
		for option, value in order_meta_data.items():
			ord_meta_data = {
				'post_id': order_id,
				'meta_key': option,
				'meta_value': value,
			}
			all_query.append(self.import_customer_data_connector(self.create_insert_query_connector('postmeta', ord_meta_data)))

		for i in convert['items']:
			orders_item_data = {
				'order_id': order_id,
				'order_item_name': i['product']['name'],
				'order_item_type': 'line_item',
			}
			self.log(orders_item_data, "check")
			order_item_id = self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_items', orders_item_data), True, convert['id'])

		new_id_product_list = []
		new_product_name_list = []
		new_product_qty_list = []
		for count, i in enumerate(convert['items']):
			post_name = self.sanitize_title(convert['items'][count]['product']['name'])
			order_product_data = {
				'post_date': convert['created_at'] if convert['created_at'] else '',
				'post_date_gmt': convert['created_at'] if convert['created_at'] else '',
				'post_title': convert['items'][count]['product']['name'],
				'post_status': 'publish',
				'post_name': post_name,
				'post_type': 'product',
			}
			new_id_product = self.import_order_data_connector(self.create_insert_query_connector('posts', order_product_data), True, convert['id'])
			new_id_product_list.append(new_id_product)
			new_product_qty_list.append(convert['items'][count]['qty'])

		for i in new_id_product_list:
			for j in new_product_qty_list:
				orders_itemmeta_data = {
					'_product_id': i,
					'_qty': j,
				}
				for option, value in orders_itemmeta_data.items():
					ord_meta_data = {
						'order_item_id': order_item_id,
						'meta_key': option,
						'meta_value': value,
					}
					self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_itemmeta', ord_meta_data))
