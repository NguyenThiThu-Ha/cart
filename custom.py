...
class LeCartCustom(LeBasecart):
	...
	def get_taxes_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM stateTaxStates WHERE id > " + to_str(
	...
	def convert_category_export(self, category, categories_ext):
		...
		category_data['created-at'] = convert_format_time(category['date_added'])
		category_data['updated_at'] = convert_format_time(category['last_modified'])
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			#'product_category': {
			#	'type': "select",
			#	'query': "SELECT * FROM  CategoryXProduct WHERE product_id IN " + product_id_con ,
			#},
			'products_images': {
				'type': "select",
				'query': "SELECT * FROM  products WHERE products_id IN  " + product_id_con ,
			},
			#'products_meta': {
			#	'type': "select",
		...
		if (not product_ext) or product_ext['result'] != 'success':
			return response_error()
		image_ids=duplicate_field_value_from_list(product_ext['data']['products_images'], 'products_id')
		...
		product_ext_rel_queries = {
			'images': {
				'type': 'select',
				'query': "SELECT products_image FROM  products WHERE products_id IN " + image_id_con,
			},
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['price'] = to_decimal( product['products_price'] ) if product['products_price'] else 0
		product_data['weight'] = to_int( product['products_weight'] ) if to_int(product['products_weight']) == to_decimal(product['products_weight']) else to_decimal(product['products_weight'])
		#if to_int(product['active']) > 0:
		#	status = True
		#else:
		#	status = False
		product_data['status'] = True if to_int(product['products_status']) == 1 else False
		#product_data['manage_stock'] = True
		product_data['qty'] = product['products_quantity']
		product_data['length'] = to_int(product['products_length']) if to_int(product['products_length']) == to_decimal(product['products_length']) else to_decimal(product['products_length'])
		product_data['width'] = to_int(product['products_width']) if to_int(product['products_width']) == to_decimal(product['products_width']) else to_decimal(product['products_width'])
		product_data['height'] = to_int(product['products_height']) if to_int(product['products_height']) == to_decimal(product['products_height']) else to_decimal(product['products_height'])
		#product_data['date_available'] = product['date_available']
		product_data['created_at'] = convert_format_time(product['products_date_added'])
		product_data['updated_at'] = convert_format_time(product['products_last_modified'])
		...
		# product_data['tags'] = product_description_def['tag']
		url_product_image = 'http://localhost/customcart1/images/'
		...
		if product_images:
			if check_thumbnail:
				for product_image in product_images:
					image_data = get_row_from_list_by_field(products_ext_data['products_images'], 'products_id', product_image['products_id'])
				...
				for product_image in product_images:
					if i == 0:
						image_data = get_row_from_list_by_field(products_ext_data['products_images'], 'id', product_image['products_id'])
						product_data['thumb_image']['url'] = url_product_image
		...
		if address_books:
			for address_book in address_books:
				...
				if country:
					...
					address_data['country']['countries_code'] = 'US'
					address_data['country']['name'] = 'United States'
				state_id = address_book['entry_state']
				if state_id:
					state = get_row_from_list_by_field(customers_ext['data']['states'], 'zone_code', address_book['entry_state'])
					if state:
						address_data['state']['id'] = state['zone_country_id']
						address_data['state']['state_code'] = state['zone_code']
						address_data['state']['name'] = state['zone_name']
					else:
						address_data['state']['state_code'] = 'AL'
				else:
					state = get_row_from_list_by_field(customers_ext['data']['states'], 'zone_id', address_book['entry_zone_id'])
					if state:
						address_data['state']['id'] = state['zone_country_id']
						address_data['state']['state_code'] = state['zone_code']
						address_data['state']['name'] = state['zone_name']
	...
	def convert_order_export(self, order, orders_ext):
		...
		for order_product in order_products:
			...
			order_item_tax = to_decimal(order_item_subtotal) * to_decimal(order_product['products_tax'])/100
	...
	def get_reviews_ext_export(self, reviews):
		...
		reviews_ext_queries = {
			...
			'reviews_description': {
				'type': 'select',
				'query': "SELECT * FROM reviews_description WHERE reviews_id IN " + self.list_to_in_condition(
					reviews_id)
			},
		}
